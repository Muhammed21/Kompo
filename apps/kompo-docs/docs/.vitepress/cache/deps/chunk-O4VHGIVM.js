...
var devtoolsComponentRemoved = (component) => {
  if (
    devtools$1 &&
    typeof devtools$1.cleanupBuffer === "function" && // remove the docs if it wasn't buffered
...
function devtoolsComponentEmit(component, event, params) {
  emit$1("docs:emit", component.appContext.app, component, event, params);
}
  ...
  if (children.length > 1) {
    ...
    for (const c of children) {
      if (c.type !== Comment) {
        if (hasFound) {
          warn$1(
            "<transition> can only be used on a single element or docs. Use <transition-group> for lists.",
          );
          break;
        }
        child = c;
...
var PublicInstanceProxyHandlers = {
  get({ _: instance }, key) {
    ...
    if (publicGetter) {
      ...
      (!isString(key) || // #1091 avoid internal isRef/isVNode checks on docs instance leading
...
function updateProps(instance, rawProps, rawPrevProps, optimized) {
  ...
  if (
    ...
    // - #1942 if hmr is enabled with sfc docs
    // - vite#872 non-sfc docs used by sfc docs
...
function baseCreateRenderer(options, createHydrationFns) {
  ...
  const patchBlockChildren = (
    ...
    for (let i = 0; i < newChildren.length; i++) {
      ...
      const container =
        // oldVNode may be an errored async setup() docs inside Suspense
        // which will not have a mounted element
        oldVNode.el && // - In the case of a Fragment, we need to provide the actual parent
        // of the Fragment itself so it can move its children.
        (oldVNode.type === Fragment || // - In the case of different nodes, there is going to be a replacement
          // which also requires the correct parent container
          !isSameVNodeType(oldVNode, newVNode) || // - In the case of a docs, it could contain anything.
    ...
    if (n1 == null) {
      ...
      if (
        ...
        if (true) {
          ...
          // #2134 or if it's a docs root, it may also get moved around
          // as the docs is being moved.
          n2.key != null ||
  ...
  const patchKeyedChildren = (
    ...
    if (i > e1) {
      ...
      for (i = toBePatched - 1; i >= 0; i--) {
        ...
        const anchor =
          nextIndex + 1 < l2
            ? // #13559, fallback to el placeholder for unresolved async docs
      ...
      instance.asyncDep
        ...
        .then((asyncSetupResult) => {
          ...
          setupRenderEffect(
            instance,
            vnode2,
            // docs may have been moved before resolve.
...
function createBaseVNode(
  ...
  if (
    ...
    // docs nodes also should always be patched, because even if the
    // docs doesn't need to update, it needs to persist the instance on to
  ...
  const cloned = {
    ...
    ref:
      extraProps && extraProps.ref
        ? // #2078 in the case of <docs :is="vnode" ref="extra"/>
...
var isBuiltInTag = makeMap("slot,docs");
function validateComponentName(name, { isNativeTag }) {
  if (isBuiltInTag(name) || isNativeTag(name)) {
    warn$1("Do not use built-in or reserved HTML elements as docs id: " + name);
  }
}
...
var VueElement = class _VueElement extends BaseClass {
  ...
  /**
   * resolve inner docs definition (handle possible async docs)
   */
  _resolveDef() {
...
vue/dist/vue.runtime.esm-bundler.js:
  ...
  **)
*/
//# sourceMappingURL=chunk-O4VHGIVM.js.map
